...
the pooled state from it) and validates it against a shared PyTorch reference.

An optional SGLang baseline (seg_la_mtp_kernel + fused_mamba_state_scatter_with_mask)
is compared when available — SGLang is not required. If it cannot be imported the
sg_* columns show nan and only the cuLA path is benchmarked. Set
LA_SGLANG_PYTHON=/path/to/sglang/python to point at a custom checkout.
...
# SGLang is an OPTIONAL comparison baseline — not every developer has it checked
# out. We try to import it (honoring LA_SGLANG_PYTHON for a custom path); if it is
# unavailable, the benchmark still runs against the PyTorch reference and simply
# skips the sg_* columns. Mirrors the pattern in bench_kda_decode_mtp.py.
